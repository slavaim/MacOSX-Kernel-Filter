...
#include "DldCommon.h"
#include "DldMsToMacTypes.h"

#ifndef _DLDDVDWHITELIST_H
#define _DLDDVDWHITELIST_H

//--------------------------------------------------------------------

//
// the following are definitions extracted from MS WDK'd headers,
// they are used for Windows DL so should be used here
//
#if 0
#ifndef __NTDDCDRM_H
#define __NTDDCDRM_H
...
//
// despite its "DVD" name the class is also used for CD drives
//
class DldDVDWhiteList: public OSObject{
    
    OSDeclareDefaultStructors( DldDVDWhiteList )
    
protected:
    
    typedef struct _wrCompletionData{
        
#if DBG
        #define  WR_SIGN  0xABCD1234
        UInt32   signature;
#endif//DBG
        UInt32   event;
        IOReturn status;
    ...
    static void asyncReadWriteCompletion(
                                         __in void *   target,
                                         __in void *   parameter,
    ...
    typedef enum _TraversingDirection{
        kTD_Any = 0x0,
        kTD_Parent,
        kTD_Child
    } TraversingDirection;
    
    static IOService* referenceFirstObjectInStackByClassName(
                                                            __in IOService*    childObject,
                                                            __in const char*   className,
                                                            __in TraversingDirection td = kTD_Any
                                                            );
    
    //
    // the WIN_ prefix marks the functions which emulate the MS Windows behavior,
    // the online DDK( include/ddk ) and some sourced of a Windows clone ( ReactOS )
    // can be found here http://www.koders.com/info.aspx?c=ProjectInfo&pid=1REGRDBN4LVVRGHNUE1HMCU5DD ,
    // also see http://msdn.microsoft.com/ for some structures definitions
    //
    virtual IOReturn WIN_ReadTOC( 
    ...
    virtual IOReturn WIN_GetCdRomGeometry(
                                   __in    IOCDBlockStorageDevice*  device,
                                   __inout DISK_GEOMETRY*           PtrDiskGeometry );
    
    
    virtual IOReturn WIN_CdReadSectors(
    ...
    virtual IOReturn WIN_ProcessTocAndConvertToLbaInLittleEndian(
                                   __in    IOCDBlockStorageDevice*  device,
                                   __inout CDROM_TOC*               CdromToc,
                                   __out   PULONG                   DiskFlags
                                   );
    
    virtual IOReturn initCDDVDDiskID( 
                                   __in IOCDBlockStorageDevice*  device, 
...
public:
    ...
    static DldDVDWhiteList*  withDefaultSettings();
    
    //
    // the function returns a media's UID by reading the media surface,
    // so the funcon is very resources and time consuming so it
    // might block for a significant amount of time, up to dozens of seconds(!),
    // a caller must allocate a space for (*diskID)
    //
    virtual IOReturn  getMediaUID( __in IOService* media, __inout CDDVDDiskID*  diskID );
};

//--------------------------------------------------------------------

extern DldDVDWhiteList*    gCDDVDWhiteList;

//--------------------------------------------------------------------

#endif// _DLDDVDWHITELIST_H
